    ...
    if (sensor_name .eq. 'AMSR2') then
        ...
        9033 format('/mnt/ops1p-ren/l/ACCESS/resampling/',a,'/source_gains_v3/g_everywhere/g_everywhere_',a,'_band_',i2.2,'.v2.dat')
        write(output_file_text,9034) trim(sensor_name),trim(sensor_name),ifreq
        9034 format('/mnt/ops1p-ren/l/ACCESS/resampling/',a,'/source_gains_v3/g_everywhere/g_everywhere_',a,'_band_',i2.2,'.v2.txt')
    endif

    if (sensor_name .eq. 'SSMIS') then
        write(source_location_file,9123) trim(sensor_name),ksat,ksat,ifreq
        9123 format('/mnt/ops1p-ren/l/ACCESS/resampling/',a,'/f',i2.2,'/source_gains/locs/find_source_gain_f' &
                     ,i2.2,'_band_',i2.2,'_locs.txt')
        write(output_file,9133) trim(sensor_name),ksat,trim(sensor_name),ifreq
        9133 format('/mnt/ops1p-ren/l/ACCESS/resampling/',a,'/f',i2.2,'/source_gains/g_everywhere/g_everywhere_' &
                     ,a,'_band_',i2.2,'.dat')
        write(output_file_text,9134) trim(sensor_name),ksat,trim(sensor_name),ifreq
        9134 format('/mnt/ops1p-ren/l/ACCESS/resampling/',a,'/f',i2.2,'/source_gains/g_everywhere/g_everywhere_' &
        ,a,'_band_',i2.2,'.txt')
    endif
