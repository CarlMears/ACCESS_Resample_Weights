    ...
    integer(int32) :: max_pixels
        ...
        !scan sizes
    integer(int32),parameter :: num_target_scans = 2  ! 1 extra scan
    integer(int32),parameter :: num_target_fovs = 446 ! 4 extra FOVS
    integer(int32),parameter :: num_source_scans = 57 ! +/- 28 scans
    integer(int32),parameter :: num_source_fovs = 90
    ...
    if (ifreq .lt. 2) error stop 
    if (ifreq .gt. 4) error stop 
    ...
    9077 format('/mnt/ops1p-ren/l/ACCESS/resampling/SSMIS/f',i2.2, &
    ...
    9013 format('/mnt/ops1p-ren/l/ACCESS/resampling/SSMIS/f',i2.2,'/target_gains/locs/find_target_gain_f', &
                        i2.2,'_circular.loc')

    write(source_location_file,9023) ksat,ksat,ifreq
    9023 format('/mnt/ops1p-ren/l/ACCESS/resampling/SSMIS/f',i2.2,'/source_gains/locs/find_source_gain_f', &
    ...
    9033 format('/mnt/ops1p-ren/l/ACCESS/resampling/SSMIS/f',i2.2,'/source_gains/g_everywhere/g_everywhere_SSMIS_band_',i2.2,'.dat')
    open(unit=3, &
    ...
    do target_scan_to_do = 1,num_target_scans
        ...
        call read_gain_target_ssmis(footprint_size_int,     &
                                    ksat,                   &
                                    ifreq,                  &
                                    target_scan_to_do,      &
                                    target_fov_to_do,       &
        ...
        do i = 1,source_locs%num_footprints         !loop over the entire source grid
            ...
            if (.not. found_source) then
                call read_gain_obs_ssmis(ksat,ifreq,iscan,icel,source_gain)
        ...
        9001 format('/mnt/ops1p-ren/l/access/resampling/ssmis/f',I2.2,'/resample_gains/',i2.2,&
                 'km/band_',i2.2,'/s',i2.2,'c',i3.3,'.dat')
    ...
    555 format('/mnt/ops1p-ren/l/ACCESS/resampling/SSMIS/f',i2.2,'/resample_weights/',i2.2,&
