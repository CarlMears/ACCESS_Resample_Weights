    include 'o:\mwi\routines\mwi_sensor_module.f'   !contains info about cell per scan, sample period, channels, beta, alpha_start    
    include 'o:\mwi\routines\mwi_simulator_module.f'  ! L1a orbit info, plus some other stuff.
    ...
    integer(4) itrg,iscan0
    real(4) xlat0,xlon0
    
    do itrg=1,4
        
        write(filename,9000) itrg
        9000 format('O:\mwi\resampling\mwi_target_arrays\find_target_gains_',i1,'.lis')
        open(6,file=filename, status='new')

        !find the scan where the simulator file crosses the equator
        ...
        ! loads scan_time,scan_rpm,scpos_eci2000,scvel_eci2000,scrpy into l2 module 
        ...
        call mwi_geolocation(iorbit)  !I think that this is called mostly ot 
        call mwi_gains(itrg,iscan0,xlat0,xlon0)
        close(6)
    enddo  !itrg
    ...
    subroutine mwi_gains(itrg,iscan0,xlat0,xlon0)
    use l2_module                                                           
    implicit none
...
!     real(8) ha(3),va(3),xant,yant,elv,azm  !not need for these target gains
...
!     ========================================================================================================================
    ...
    call openbig(3,'O:\mwi\antenna\data\target_beamwidth_sumgain.dat','old')  !antenna pattern from Ball?
    read(3) beamwidth
    read(3) sumgain
    close(3)
    ...
    do kscan=kscan1,kscan2
        ...
        do icel=1,2*maxcel-1
            ...
            !     drapers pattern already include pattern smearing from integration 
            !     so i do not need the     do interval=0,9 loop
            ...
            !     next two statements not needed for these gain targets
            !     call cross_norm(b,z_eci, ha)      !ha=kx(-z)=(-b)x(-z)=bxz
            !    call cross_norm(b,ha,    va)     !va=haxk=bxha

            !     ==============================================================================================================================    
            !      compute the vector from the earth's center that passes through the current cell locations 
            !      and goes to alttitude of satellite
            !     ==============================================================================================================================    
            ...
            do ilat=1,nlat
                do ilon=1,nlon
                    ...
                    if(delta.le.3*beamwidth(itrg)) then  !see mathnotes
                         call fd_gain_targets(beamwidth(itrg),delta, gain)
            ...
            write(filename,9001) itrg,kscan+4,icel
            9001 format('O:\mwi\resampling\mwi_target_arrays\target',i1,'\s',i2.2,'c',i4.4,'.dat')
