...
c     real(8), parameter ::   smooth_fac=1.0d-06  
      real(8) smooth_fac  ! smoothtest  
	...
	close(3)
      ...
      if(icase.eq.1) then
	open(6,   file='data\find_resampling_weights_leakage_smoothtest_4stk.lis',status='new')
      call openbig(4,'data\find_resampling_weights_leakage_smoothtest_4stk.dat','new')
      icase_trg= 1 !first stokes
      icase_obs0=3
      endif
      
      if(icase.eq.2) then
	open(6,   file='data\find_resampling_weights_leakage_smoothtest_3stk.lis',status='new')
      call openbig(4,'data\find_resampling_weights_leakage_smoothtest_3stk.dat','new')
      icase_trg= 2 !second stokes
      icase_obs0=5
      endif
      
      if(icase.eq.3) then
 	open(6,   file='data\find_resampling_weights_leakage_smoothtest_2stk.lis',status='new')
      call openbig(4,'data\find_resampling_weights_leakage_smoothtest_2stk.dat','new')
      icase_trg= 2 !second stokes
      icase_obs0=7
      endif

	do ismooth=1,4 
      if(ismooth.eq.1) smooth_fac=1.0d-03 
      if(ismooth.eq.2) smooth_fac=1.0d-04 
      if(ismooth.eq.3) smooth_fac=1.0d-06  !
      if(ismooth.eq.4) smooth_fac=1.0d-09  
...
c	do jcel=1,maxcel !this is the target 
	do jcel=285,285  !this is the target, smoothtest 
